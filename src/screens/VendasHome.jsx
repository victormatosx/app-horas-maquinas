...
// Importar o componente de geração de relatórios
import { ReportGenerator, PDFManager } from "./ReportGenerator"
...
// Substituir a função sanitizeDataForFirebase pela versão mais simples e segura:

// Função para limpar dados antes de salvar no Firebase - VERSÃO SEGURA
const sanitizeDataForFirebase = (data) => {
  // Se for null, retorna null (Firebase aceita)
  if (data === null) {
    return null
  }

  // Se for undefined, retorna null (Firebase não aceita undefined)
  if (data === undefined) {
    return null
  }

  // Se for um objeto (mas não array)
  if (typeof data === "object" && !Array.isArray(data)) {
    const cleaned = {}
    for (const [key, value] of Object.entries(data)) {
      // Recursivamente limpa cada propriedade
      cleaned[key] = sanitizeDataForFirebase(value)
    }
    return cleaned
  }
  ...
  // Para todos os outros tipos (string, number, boolean), retorna como está
  return data
}
...
// Componente para o Modal de Menu de Ações
const ActionsMenuModal = ({ visible, onClose, onReportPress, onAddClientPress, onNewSalePress }) => {
  const menuOptions = [
    {
      id: "new-sale",
      title: "Nova Venda",
    ...
    },
    {
      ...
      subtitle: "Emitir relatório de vendas",
      icon: "assessment",
  ...
  return (
    <Modal visible={visible} transparent={true} animationType="fade">
      <TouchableOpacity style={styles.menuModalOverlay} activeOpacity={1} onPress={onClose}>
        <View style={styles.menuModalContainer}>
          ...
          <View style={styles.menuOptions}>
            {menuOptions.map((option) => (
              <TouchableOpacity key={option.id} style={styles.menuOption} onPress={() => handleOptionPress(option)}>
                <View style={[styles.menuOptionIcon, { backgroundColor: `${option.color}15` }]}>
...
// Componente para o Modal de Adicionar Cliente
const AddClientModal = ({ visible, onClose, onClientAdded, propriedadeNome }) => {
  ...
  const handleSaveClient = async () => {
    ...
    setIsLoading(true)
    try {
      const clientsRef = ref(database, `propriedades/${propriedadeNome}/clientes`)
      const newClientRef = push(clientsRef)

      const clientData = sanitizeDataForFirebase({
        Codigo: clientCode.trim(),
  ...
  return (
    <Modal visible={visible} transparent={true} animationType="slide">
      <View style={styles.modalOverlay}>
        <View style={styles.addClientModalContainer}>
          <View style={styles.addClientModalHeader}>
            <Text style={styles.addClientModalTitle}>Adicionar Novo Cliente</Text>
            <TouchableOpacity onPress={handleClose} style={styles.addClientCloseButton}>
              <Icon name="close" size={24} color={COLORS.gray600} />
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.addClientForm}>
            <View style={styles.inputGroup}>
              <Text style={styles.inputLabel}>Código do Cliente *</Text>
              <TextInput
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="Ex: A Casa da Cenoura Ltda"
                placeholderTextColor={COLORS.gray400}
                maxLength={100}
                multiline={true}
                numberOfLines={2}
          ...
          <View style={styles.addClientActions}>
            <TouchableOpacity
              style={[styles.addClientButton, styles.cancelButton]}
              onPress={handleClose}
              disabled={isLoading}
            >
            ...
            <TouchableOpacity
              style={[styles.addClientButton, styles.saveButton]}
...
const SaleDetailsModal = ({ visible, onClose, saleData, propriedadeNome, onEdit, onCancel, onReactivate, logoUri }) => {
  ...
  const handleGenerateAndSharePDF = async () => {
    ...
    try {
      const pdfUri = await PDFManager.generateSalesReport(saleData, propriedadeNome, logoUri, false)
      if (pdfUri) {
  ...
  const handleCancelSale = () => {
    if (!saleData || saleData.status === "cancelada") return

    Alert.alert(
      "Cancelar Venda",
      "Tem certeza que deseja cancelar esta venda? Esta ação pode ser revertida posteriormente.",
      [
        {
          text: "Não",
          style: "cancel",
  ...
  const handleReactivateSale = () => {
    if (!saleData || saleData.status !== "cancelada") return

    Alert.alert("Reativar Venda", "Tem certeza que deseja reativar esta venda cancelada?", [
      {
        text: "Não",
        style: "cancel",
      },
      {
        ...
        onPress: async () => {
          ...
          try {
            await onReactivate(saleData.id)
            onClose()
          } catch (error) {
  ...
  const isCanceled = saleData.status === "cancelada"
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.detailsModalContainer}>
            ...
            <View style={styles.detailsModalTitleContainer}>
              ...
              {isCanceled && (
                <View style={styles.canceledBadge}>
                  ...
                  <Text style={styles.canceledBadgeText}>CANCELADA</Text>
                </View>
              )}
            </View>
          ...
          <ScrollView style={styles.detailsModalContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={[styles.detailsSection, isCanceled && styles.canceledSection]}>
              ...
              <View style={styles.detailsGrid}>
                ...
                <View style={styles.detailsItem}>
                  ...
                  <View style={styles.statusContainer}>
                    <Text
                      style={[styles.detailsValue, isCanceled ? styles.canceledStatusText : styles.activeStatusText]}
                    >
                      {isCanceled ? "Cancelada" : "Ativa"}
                    </Text>
                    {isCanceled && <MaterialIcons name="cancel" size={16} color={COLORS.canceled} />}
                  </View>
                </View>
                <View style={styles.detailsItem}>
                ...
                {isCanceled && saleData.canceladoEm && (
                  <View style={styles.detailsItem}>
                    <Text style={styles.detailsLabel}>Cancelado em:</Text>
                    <Text style={styles.canceledDateText}>
                      {new Date(saleData.canceladoEm).toLocaleString("pt-BR")}
                    </Text>
                  </View>
                )}
              </View>
            ...
            <View style={[styles.detailsSection, isCanceled && styles.canceledSection]}>
              ...
              {saleData.itens && saleData.itens.length > 0 ? (
                saleData.itens.map((item, index) => (
                  <View key={index} style={[styles.itemDetailsCard, isCanceled && styles.canceledItemCard]}>
            ...
            {(saleData.observacao || saleData.observacaoPagamento) && (
              <View style={[styles.detailsSection, isCanceled && styles.canceledSection]}>
                ...
                {saleData.observacaoPagamento && (
                  <View style={[styles.observationCard, isCanceled && styles.canceledObservationCard]}>
            ...
            {/* Total */}
            <View style={[styles.totalSection, isCanceled && styles.canceledTotalSection]}>
              <Text style={styles.totalLabel}>Total da Venda:</Text>
              <Text style={[styles.totalValue, isCanceled && styles.canceledTotalValue]}>
                R$ {saleData.valorTotal ? saleData.valorTotal.toFixed(2) : "0,00"}
              </Text>
            </View>
          ...
          <View style={styles.detailsActions}>
            {!isCanceled ? (
              <>
                ...
                <TouchableOpacity
                  style={[styles.actionButton, styles.cancelSaleButton]}
...
export default function VendasHome() {
  ...
  useEffect(() => {
    const loadUserDataAndLogo = async () => {
      try {
        ...
        setLogoUri(MATRICE_LOGO_BASE64)
      } catch (error) {
  ...
  const handleEditSale = (saleData) => {
    if (saleData.status === "cancelada") {
      Alert.alert("Aviso", "Não é possível editar uma venda cancelada. Reative a venda primeiro.")
      return
    }
  ...
  const handleCancelSale = async (saleId) => {
    try {
      const saleRef = ref(database, `propriedades/${propriedadeNome}/vendas/${saleId}`)
      const updateData = sanitizeDataForFirebase({
        status: "cancelada",
      ...
      console.error("Erro ao cancelar venda:", error)
      Alert.alert("Erro", "Não foi possível cancelar a venda. Tente novamente.")
      throw error
    }
  }

  const handleReactivateSale = async (saleId) => {
    try {
      const saleRef = ref(database, `propriedades/${propriedadeNome}/vendas/${saleId}`)
      const updateData = sanitizeDataForFirebase({
        status: "ativa",
      ...
      Alert.alert("Sucesso", "Venda reativada com sucesso!")
    } catch (error) {
      console.error("Erro ao reativar venda:", error)
      Alert.alert("Erro", "Não foi possível reativar a venda. Tente novamente.")
      throw error
    }
  }
  ...
  const renderSaleItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.saleItemCard, isCanceled && styles.canceledSaleCard]}
        onPress={() => handleSalePress(item)}
      >
        {isCanceled && (
          <View style={styles.canceledOverlay}>
            <MaterialIcons name="cancel" size={24} color={COLORS.canceled} />
            <Text style={styles.canceledOverlayText}>CANCELADA</Text>
          </View>
        )}

        <View style={styles.saleItemHeader}>
          <Text style={[styles.saleItemTitle, isCanceled && styles.canceledSaleTitle]}>
            {item.cliente || "Cliente Desconhecido"}
          </Text>
          {/* Removido o statusContainer para evitar duplicação - só mostra o badge se NÃO for cancelada */}
          {!isCanceled && (
            <View style={styles.statusContainer}>
              <View
                style={[
        ...
        <View style={styles.saleItemFooter}>
          <Text style={[styles.saleItemTotal, isCanceled && styles.canceledSaleTotal]}>
            {formatCurrencyBRL(item.valorTotal)}
          </Text>
          <Icon name="chevron-forward" size={20} color={isCanceled ? COLORS.gray400 : COLORS.primary} />
        </View>
      </TouchableOpacity>
    )
  }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.menuButton} onPress={() => setShowActionsMenu(true)}>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder="Buscar por cliente, data, forma de pagamento ou status..."
            value={searchQuery}
      ...
      <ActionsMenuModal
        ...
        onReportPress={() => setShowReportGenerator(true)}
        onAddClientPress={() => setShowAddClientModal(true)}
        onNewSalePress={() => navigation.navigate("SalesScreen")}
      />
...
const styles = StyleSheet.create({
  ...
  canceledSaleCard: {
    ...
    borderWidth: 1,
    borderColor: COLORS.canceledBorder,
    elevation: 1,
  },
  canceledOverlay: {
    position: "absolute",
  ...
  saleItemHeader: {
    ...
    alignItems: "center",
    marginBottom: 12,
    paddingRight: 80, // Espaço para o overlay de cancelado
  },
  saleItemTitle: {
    fontSize: 16,
    fontWeight: "bold",
  ...
  // Modal de Adicionar Cliente Styles
